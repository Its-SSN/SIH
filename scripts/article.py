...
def get_links(q = "government policies, programs, initiatives, and achievements in India"):
    gn = GoogleNews()
    results = gn.search(q)
    links=[]
...
def article_scrapper(q = "government policies, programs, initiatives, and achievements in India"):
    articles = []
    for link in get_links(q):
        try:
            article = newspaper.Article(url=link, language='en')
            article.download()
            article.parse()

            article ={"Text": "".join(str(article.text).split('\n')),
                      "Type": "article",
